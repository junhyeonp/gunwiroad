...
export const qrCheck = async (request, response) => {
    ...
    const course = await db
        .execute(QUERY1, [qrInfoData.qrCode])
        .then((result) => result[0][0]);
    if (!course)
        return response
            .status(400)
            .json({ status: "올바른 QR코드가 아닙니다." });

    // 검증코드 2: 해당유저 이 코스에 방문한적이 있는지
    const QUERY2 = `SELECT * FROM users_course WHERE user_id = ? AND course_id = ?`;
    const userVisited = await db
        .execute(QUERY2, [userId, course.course_id])
        .then((result) => result[0][0]);

    if (userVisited)
        return response.status(400).json({ status: "이미 방문한 장소입니다." });

    console.log("성공");

    // 검증코드 3 (수학) : 반경 100m내에 있을때만 qr코드 찍을 수 있음 - 선택
    // dist m로 나옴
    const dist = calculatorDistance(
        qrInfoData.latitude,
        qrInfoData.longitude,
        course.latitude,
        course.longitude
    );

    if (dist > 100)
        return response.status(400).json({ status: "거리가 너무 멉니다." });
...
export const favorite = async (req, res) => {
    ...
    const sql =
        "INSERT INTO users_course_favorite (user_id, course_id) VALUES (?, ?)";
...
export const removeFavorite = async (req, res) => {
    ...
    const sql =
        "DELETE FROM users_course_favorite WHERE user_id = ? AND course_id = ?";
    db.query(sql, [userId, courseId], (err, result) => {
        if (err) {
            console.error(err);
